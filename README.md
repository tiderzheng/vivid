...
- **本地文件**
  - 直接使用本地视频 / 音频
- **Bilibili**
  - `auto` / `smart` 模式优先提取官方字幕
  - 没有可用字幕或字幕接口失败时下载媒体，再走 Whisper / OCR
...
同时还会生成一组面向后续向量化 / embedding / RAG 的程序消费产物：
...
- `data/项目名/artifacts/`：给人读
- `data/项目名/vector_source/`：给程序做后续向量化入库
