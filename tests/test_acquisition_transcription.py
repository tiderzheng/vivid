...
def test_acquire_transcript_smart_keeps_bilibili_subtitle_priority(tmp_path, monkeypatch):
    workdir = tmp_path / "work"
    options = _build_options(tmp_path, backend="auto", acquisition_mode="smart")
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: "官方字幕",
    )
    monkeypatch.setattr(
        "app.pipeline.acquisition.detect_hard_subtitles",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("should not probe when subtitle already available")),
    )
    monkeypatch.setattr(
        "app.pipeline.acquisition.create_media_path",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("should not download media when subtitle already available")),
    )

    result = acquire_transcript(options, "bilibili", workdir)
    assert result.text == "官方字幕"
    assert result.acquisition_method == "Bilibili subtitle"


def test_acquire_transcript_reports_subtitle_failure_reason(tmp_path, monkeypatch):
    workdir = tmp_path / "work"
    ...
    monkeypatch.setattr(
        "app.pipeline.acquisition.BilibiliAdapter.export_subtitles",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("subtitle broken")),
    )
    monkeypatch.setattr("app.pipeline.acquisition.create_media_path", lambda *_args, **_kwargs: media_path)
    monkeypatch.setattr(
        "app.pipeline.acquisition.InternalTranscriptionEngine.transcribe",
        lambda *_args, **_kwargs: InternalTranscriptionResult(
            transcript="ok",
    ...
    result = acquire_transcript(options, "bilibili", workdir, event_callback=lambda stage, message, data=None: events.append((stage, message, data)))
