...
def test_vivid_tool_ps1_accepts_legacy_sessdata_flags_without_forwarding(tmp_path):
    ...
    assert "--sessdata" not in recorded
    assert "--no-sessdata" not in recorded
    assert "-m app.control_cli quickread" in recorded


@pytest.mark.skipif(_powershell_executable() is None, reason="PowerShell is not available")
def test_run_quickread_ps1_accepts_legacy_sessdata_flags_without_forwarding(tmp_path):
    repo_root = tmp_path / "repo"
    scripts_dir = repo_root / "scripts"
    scripts_dir.mkdir(parents=True, exist_ok=True)
